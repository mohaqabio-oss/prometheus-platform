...
import {
  ...
  Clock, Circle, Trash2, Edit3, Calendar,
} from "lucide-react";
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6B7280] font-sans mt-1">
            إدارة مشاريع بروميثيوس البحثية وتعيين الفرق وربط المنشورات.
          </p>
        </div>
      ...
      {projects.length === 0 ? (
        <div className="text-center py-24 rounded-2xl border border-dashed border-[#6B7280]/30 bg-[#0D0D0D]">
          ...
          <p className="text-[#6B7280] font-cairo text-lg font-bold">لا توجد مشاريع بعد</p>
          <p className="text-sm text-[#6B7280]/60 mt-2">ابدأ بإنشاء أول مشروع بحثي لفريق بروميثيوس.</p>
          <Link href="/admin/projects/new">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {projects.map((project) => {
            ...
            return (
              <div key={project.id}
                className="bg-[#0D0D0D] border border-[#6B7280]/20 rounded-2xl overflow-hidden hover:border-[#E84A0C]/30 transition-all duration-200 group shadow-lg">
                {/* Cover */}
                {project.coverImage ? (
                ...
                <div className="p-5 space-y-3">
                  ...
                  {/* Stats */}
                  <div className="flex items-center gap-4 text-[11px] text-[#6B7280] font-fira pt-1">
                  ...
                  {/* Actions */}
                  <div className="flex gap-2 pt-2 border-t border-[#6B7280]/20">
                    <Link href={`/admin/projects/${project.id}/edit`} className="flex-1">
                      <Button variant="ghost" size="sm"
                        className="w-full gap-1.5 text-xs text-[#6B7280] hover:text-white hover:bg-[#1A2B4A]">
                        <Edit3 className="w-3.5 h-3.5 text-[#E84A0C]" /> تعديل
                      </Button>
                    </Link>
