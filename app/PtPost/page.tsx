...
export default async function PrometheusPostHomePage() {
  // Query ONLY articles with type === "ACADEMIC" and status === "PUBLISHED"
  let academicArticles: any[] = [];
  let totalAcademicCount = 0;
  let totalReviewersCount = 0;

  try {
    academicArticles = await prisma.article.findMany({
      where: {
        status: "PUBLISHED",
    ...
    totalAcademicCount = await prisma.article.count({
      where: {
        status: "PUBLISHED",
        type: ArticleType.ACADEMIC,
      },
    ...
    totalReviewersCount = await prisma.editorialMember.count();
  } catch (err) {
    console.error("[PtPost_HOME_QUERY_ERROR]:", err);
  }
  ...
  return (
    <div className="py-12 sm:py-20 container mx-auto px-4 sm:px-6 md:px-8 max-w-6xl space-y-16 relative">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 relative z-10">
        <Card className="p-5 bg-[#0D1322]/80 border border-white/10 rounded-2xl text-center space-y-1 shadow-md">
      ...
      <div className="space-y-8 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/10 pb-4">
          ...
          />
          <Link href="/PtPost/articles" className="shrink-0">
            <Button
              variant="outline"
              size="sm"
              ...
              return (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono text-stone-400">
                    ...
                    <Link
                      href={`/PtPost/articles/${article.slug}`}
                      className="flex items-center gap-1 text-[#E84A0C] hover:underline"
                    >
      ...
      <div className="p-8 sm:p-10 rounded-2xl bg-gradient-to-r from-[#141C2F] to-[#0A0F1D] border border-white/10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative z-10">
        <div className="space-y-2 text-center md:text-right">
          <Badge variant="orange">إيداع الأبحاث الأكاديمية</Badge>
          <h2 className="font-['Playfair_Display',serif] text-2xl font-bold text-white">
            هل ترغب في نشر ورقتك البحثية في Prometheus Post؟
          </h2>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <Link href="/PtPost/publication-ethics">
