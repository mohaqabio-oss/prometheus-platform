...
export default function PrometheusPostArticlesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const arts = await getPublicArticlesAction(ArticleType.ACADEMIC);
        setArticles(arts || []);
      } catch (e) {
        console.error("Error loading academic articles:", e);
        setArticles([]);
      } finally {
        setLoading(false);
      }
  ...
  const categories = ["الكل", "الهندسة البرمجية", "الذكاء الاصطناعي", "البحث العلمي", "الأمن السيبراني"];
  ...
  return (
    <div className="py-12 sm:py-20 container mx-auto px-4 sm:px-6 md:px-8 max-w-6xl space-y-12 relative animate-fade-in">
      ...
      <div className="text-center space-y-4 relative z-10 border-b border-stone-800 pb-8">
        ...
        <div className="border-y-2 border-stone-700 py-6 my-2 bg-[#0D1322]/90 backdrop-blur-xl rounded-2xl shadow-lg">
          ...
          <p className="text-xs font-mono tracking-widest text-[#E84A0C] uppercase mt-2 font-semibold">
            أرشيف ومجموعات الأوراق البحثية والمراجعات الأكاديمية المحكمة
          </p>
        </div>
      </div>
      ...
      {loading ? (
              ...
              <Card className="h-full p-6 bg-[#0D1322]/80 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col justify-between space-y-4 shadow-xl hover:shadow-2xl hover:border-[#E84A0C]/50 transition-all duration-300 relative overflow-hidden group">
                ...
                <div className="space-y-3 relative z-10">
                  ...
                  <h3 className="font-['Playfair_Display',serif] text-lg font-bold text-white group-hover:text-[#E84A0C] transition-all duration-300">
                    <Link href={`/PtPost/articles/${article.slug}`}>
                      {article.title}
                    </Link>
                  </h3>
                ...
                <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono text-stone-400 relative z-10">
                  <span>{article.authorName || article.author?.fullName || "محرر بروميثيوس"}</span>
        ...
        <Card className="p-12 text-center border border-dashed border-white/15 bg-[#0D1322]/80 backdrop-blur-xl space-y-3">
          ...
          <p className="text-xs text-stone-400 max-w-md mx-auto">
            سيتم نشر المقالات والأوراق البحثية المعتمدة فور صدورها من قبل الهيئة التحريرية للمجلة.
          </p>
        </Card>
      )}
