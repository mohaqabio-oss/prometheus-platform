...
export function NewspaperArticleView({
  article,
  latestArticles = [],
  backLinkHref = "/blog",
  backLinkLabel = "العودة إلى الصفحة الرئيسية",
}: NewspaperArticleViewProps) {
  const authorsList =
    article.authors && article.authors.length > 0
      ...
      : [
          {
            id: "default-author",
            fullName: article.authorName || "محرر بروميثيوس",
  ...
  return (
    <article className="py-10 sm:py-16 bg-[#FCFBF9] min-h-screen !text-stone-900 animate-fade-in font-sans selection:bg-red-200 selection:!text-red-900">
      ...
      {/* Google Fonts for Classic Newspaper Typography */}
      <link
        rel="stylesheet"
        ...
        <div className="mb-8 flex items-center justify-between border-b border-stone-200 pb-4">
          <Link href={backLinkHref}>
            <Button variant="ghost" size="sm" className="gap-2 !text-stone-700 hover:!text-black hover:bg-stone-200/50 transition-colors">
              ...
              <span className="font-semibold">{backLinkLabel}</span>
            </Button>
          </Link>

          <span className="text-[11px] font-mono !text-stone-600 uppercase tracking-widest hidden sm:inline-block font-bold">
            The Prometheus Post • Academic & Technology Chronicle
          </span>
        </div>

        {/* Classic Editorial Newspaper Masthead */}
        <header className="mb-12 text-center">
          ...
          <div className="py-8 my-1 bg-transparent">
            ...
            <p className="text-xs font-mono tracking-widest !text-red-800 uppercase mt-3 font-bold">
              The Official Journal & Technology Chronicle of Prometheus Platform
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between text-[10px] font-mono uppercase tracking-widest !text-stone-700 pt-2 border-t-2 border-black font-bold">
            <span>{article.type === "ACADEMIC" ? "PEER-REVIEWED RESEARCH" : "EDITORIAL & ESSAYS"}</span>
          ...
          <main className="lg:col-span-8 space-y-10">
            ...
            <div className="border-b border-stone-300 pb-8">
              <span className="bg-red-800 !text-white px-3 py-1 text-[11px] uppercase font-bold tracking-widest inline-block font-mono mb-6 shadow-sm">
                {article.category?.name || (article.type === "ACADEMIC" ? "FEATURED ACADEMIC JOURNAL" : "PUBLIC ESSAY & CHRONICLE")}
              </span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <strong className="!text-black font-bold uppercase tracking-wider">
                      {[...authorsList.map((au: any) => au.fullName || au.name), ...(article.guestAuthors || [])].join(" & ")}
                    </strong>
                  </div>
                </div>
              ...
              <figure className="mb-10">
                <div className="border border-stone-300 bg-white p-1 shadow-sm">
                  <img
                    ...
                    alt={article.title}
                    className="w-full h-auto max-h-[550px] object-cover grayscale-[15%] contrast-[1.05]"
                  />
                </div>
            ...
            {article.sources && article.sources.length > 0 && (
              <div className="sources-box mt-12 p-6 bg-stone-100 border border-stone-300 space-y-4">
                <h3 className="font-['Playfair_Display',serif] text-xl font-bold !text-black flex items-center gap-2 border-b border-stone-300 pb-2">
                  <span>المصادر والمراجع</span>
                </h3>
                <ol className="list-decimal list-inside space-y-2 text-xs font-mono text-stone-800">
                  {article.sources.map((src: string, i: number) => (
                    <li key={i} className="break-all">
                      {src.startsWith("http") ? (
                        <a href={src} target="_blank" rel="noopener noreferrer" className="text-red-800 hover:underline">{src}</a>
                      ) : src}
                    </li>
                  ))}
                </ol>
              ...
              <div className="pt-6 border-t-2 border-stone-200 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {article.partners.map((pa: any) => (
                    <Link
                      key={pa.partner?.id || pa.partnerId}
                      href={`/partners/${pa.partner?.slug || pa.slug}`}
                    ...
                    >
                      <div className="w-12 h-12 bg-stone-50 border border-stone-200 p-1 flex items-center justify-center shrink-0">
                        <img src={pa.partner?.logoUrl || pa.logoUrl} alt={pa.partner?.name || pa.name} className="w-full h-full object-contain" />
                      </div>
          ...
          <aside className="lg:col-span-4 space-y-10">
            ...
            <div className="border-t-4 border-black pt-4">
              <div className="flex items-center justify-between mb-6 border-b-2 border-stone-200 pb-3">
                ...
                <span className="text-[10px] font-mono !text-stone-600 uppercase font-bold">CHRONICLE</span>
              </div>
                  ...
                  latestArticles.map((item: any) => (
                    <article key={item.id} className="border-b border-stone-200 pb-6 space-y-2 group">
                      <div className="flex items-center gap-2 text-[10px] font-mono !text-stone-600 uppercase font-bold">
                        <span className="!text-red-800">{item.category?.name || item.categoryName || "ESSAY"}</span>
                        <span>•</span>
                      ...
                      <h3 className="font-['Playfair_Display',serif] text-lg font-bold !text-black group-hover:!text-red-800 transition-colors leading-snug">
                        <Link href={getArticlePublicUrl(item.slug, item.type)}>
                          {item.title}
                        </Link>
                      </h3>
